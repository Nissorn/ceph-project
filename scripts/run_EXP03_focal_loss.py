...
Hypothesis: CrossEntropy dominates the loss with background pixels.
Focal Loss will force the model to focus on the hard-to-predict thin bone plates.

Config:
  - Architecture: UNetPlusPlus (resnet50 encoder) — same as EXP-02 (best so far)
  - LR: 3e-4 (conservative)
  - Accumulation: 2 → effective batch = 4 * 4(gpu) * 2 = 32
  - Loss: DiceLoss + FocalLoss (gamma=2.0, alpha=1.0)
...
BATCH_SIZE_PER_GPU = 4        # UNet++ 49M params — safe @ 1024px with AMP
...
WEIGHT_DECAY     = 1e-3
DICE_WEIGHT      = 0.5
FOCAL_GAMMA      = 2.0         # EXP-03 key change: Focal Loss gamma
FOCAL_ALPHA      = 1.0         # EXP-03 key change: Focal Loss alpha
MAX_EPOCHS       = 150
PATIENCE         = 30
SEED             = 42          # FIXED: seed set BEFORE any random ops
...
log = logging.getLogger("EXP03_focal")
...
def patient_split(records, val_pct=0.2, seed=42):
    # FIX: set seed BEFORE any shuffling — ensures deterministic split across runs
    random.seed(seed)
    np.random.seed(seed)
...
# ═══════════════════════════════════════════════════════════════════
# LOSS — EXP-03 KEY CHANGE: FocalLoss + DiceLoss
# ═══════════════════════════════════════════════════════════════════
class FocalLoss(nn.Module):
    """Focal Loss for multi-class segmentation. Focuses on hard pixels."""
    ...
    def forward(self, pred, target):
        ...
        ce = F.cross_entropy(pred, target, reduction="none", ignore_index=0)
...
class FocalDiceLoss(nn.Module):
    """Dice + Focal Loss for EXP-03 — targets hard bone-plate pixels."""
    def __init__(self, dice_weight=0.5, focal_gamma=2.0, focal_alpha=1.0):
        super().__init__()
    ...
    def forward(self, pred, target):
        ...
        return (1.0 - self.dice_weight) * fl + self.dice_weight * (dice / (NUM_CLASSES - 1))
...
def train_one_model(model, train_dl, val_dl, epochs, run_id, patience, accumulation_steps):
    ...
    # EXP-03: FocalDiceLoss instead of CrossEntropyDiceLoss
    loss_fn = FocalDiceLoss(dice_weight=DICE_WEIGHT, focal_gamma=FOCAL_GAMMA, focal_alpha=FOCAL_ALPHA)
    scaler  = GradScaler()
...
def main():
    log.info("=" * 68)
    log.info("EXP-03: Focal Loss | UNet++ / resnet50")
    log.info("LR=%.0e | Eff.bs=%d | Focal gamma=%.1f | amp=ON | grad_acc=%d",
             BASE_LR, EFFECTIVE_BS, FOCAL_GAMMA, ACCUMULATION_STEPS)
    ...
    arch       = "UnetPlusPlus"
    run_id     = f"EXP03_FocalDice_resnet50_lr3e4_g{int(FOCAL_GAMMA)}_acc2_{RUN_TS}"
    ...
    cfg = {
        ...
        "experiment_type": "phase2c_focal_loss",
        "run_id": run_id,
        "arch_name": arch,
        "encoder_name": "resnet50",
    ...
    log.info(
        "  EXP-03 DONE | dice=%.4f | iou=%.4f | time=%.0fs | saved: %s",
