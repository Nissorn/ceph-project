...
Hypothesis: U-Net++ nested decoders might capture thin bone boundaries better than DeepLabV3+ at high resolution.

Config:
  - Architecture: UNetPlusPlus (resnet50 encoder)
  - LR: 3e-4 (conservative — EXP-01's 5e-4 showed good convergence, use 3e-4)
...
BASE_LR          = 3e-4        # EXP-02: moderate LR (down from EXP-01's 5e-4)
MIN_LR           = 1e-6
WARMUP_EPOCHS    = 10
WARMUP_START     = 1e-5
...
WEIGHT_DECAY     = 1e-3
DICE_WEIGHT      = 0.5
MAX_EPOCHS       = 150         # EXP-02: 150 (was 200 for EXP-01)
PATIENCE         = 30
...
log = logging.getLogger("EXP02_unetpp")
...
def main():
    log.info("=" * 68)
    log.info("EXP-02: U-Net++ Focus | UnetPlusPlus / resnet50")
    log.info("LR=3e-4 | Eff.bs=%d | img=%s | amp=ON | grad_acc=%d | max_ep=%d",
             EFFECTIVE_BS, IMAGE_SIZE, ACCUMULATION_STEPS, MAX_EPOCHS)
    log.info("LR: %.0e → %.0e (warmup %dep) → cosine | patience=%d",
             WARMUP_START, BASE_LR, WARMUP_EPOCHS, PATIENCE)
    log.info("=" * 68)
    ...
    arch       = "UnetPlusPlus"
    run_id     = f"EXP02_UnetPlusPlus_resnet50_lr3e4_acc2_{RUN_TS}"
    ...
    cfg = {
        ...
        "experiment_type": "phase2c_unetpp_focus",
        "run_id": run_id,
        "arch_name": arch,
        "encoder_name": "resnet50",
    ...
    log.info(
        "  EXP-02 DONE | dice=%.4f | iou=%.4f | time=%.0fs | saved: %s",
