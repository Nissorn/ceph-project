...
Hypothesis: ResNet50 is too large for 290 images and causes capacity shock.
Reverting to a smaller encoder at 1024px might generalize better.

Config:
  - Architecture: DeepLabV3+ (efficientnet-b4 encoder)
  - LR: 3e-4
  - Accumulation: 2 → effective batch = 4 * 4(gpu) * 2 = 32
  - Loss: Dice + CrossEntropy (same as EXP-01)
...
BATCH_SIZE_PER_GPU = 4        # DeepLabV3+ efficientnet-b4 — safe @ 1024px with AMP
ACCUMULATION_STEPS  = 2        # eff bs = 4 * 4 * 2 = 32
EFFECTIVE_BS     = BATCH_SIZE_PER_GPU * GPU_COUNT * ACCUMULATION_STEPS

BASE_LR          = 3e-4        # EXP-04: same as EXP-02/03
MIN_LR           = 1e-6
WARMUP_EPOCHS    = 10
WARMUP_START     = 1e-5
...
log = logging.getLogger("EXP04_effb4")
...
# ═══════════════════════════════════════════════════════════════════
# BUILD MODEL — DeepLabV3+ with efficientnet-b4 encoder
# ═══════════════════════════════════════════════════════════════════
def build_model(arch_name):
    if arch_name == "DeepLabV3Plus":
        model = smp.DeepLabV3Plus(
            encoder_name="efficientnet-b4",   # EXP-04 key change: lightweight encoder
...
def main():
    log.info("=" * 68)
    log.info("EXP-04: Lightweight Encoder | DeepLabV3+ / efficientnet-b4")
    log.info("LR=%.0e | Eff.bs=%d | amp=ON | grad_acc=%d",
             BASE_LR, EFFECTIVE_BS, ACCUMULATION_STEPS)
    ...
    arch       = "DeepLabV3Plus"
    run_id     = f"EXP04_DeepLabV3Plus_efficientnetb4_lr3e4_acc2_{RUN_TS}"
    ...
    cfg = {
        ...
        "experiment_type": "phase2c_lightweight_encoder",
        "run_id": run_id,
        "arch_name": arch,
    ...
    log.info(
        "  EXP-04 DONE | dice=%.4f | iou=%.4f | time=%.0fs | saved: %s",
