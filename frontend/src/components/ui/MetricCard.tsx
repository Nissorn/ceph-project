...
const MetricCard = memo(function MetricCard({ title, value, status = 'normal', subtitle }: MetricCardProps) {
  ...
  if (status === 'warning') {
    config = {
      ...
      borderGlow: 'hover:border-amber-300 dark:hover:border-amber-500',
    };
    ...
    config = {
      ...
      borderGlow: 'hover:border-rose-300 dark:hover:border-rose-500',
    };
  }

  return (
    <div className={`bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-5 flex flex-col relative overflow-hidden transition-all duration-300 hover:-translate-y-0.5 hover:shadow-md ${config.borderGlow} group`}>
