...
const MetricCard = memo(function MetricCard({ title, value, status = 'normal', subtitle }: MetricCardProps) {
  ...
  if (status === 'warning') {
    ...
    config = {
      ...
      valueColor: 'text-rose-600 font-normal',
  ...
  return (
    <div className={`bg-white border border-slate-200 rounded-xl p-5 flex flex-col relative overflow-hidden transition-all duration-300 hover:-translate-y-0.5 hover:shadow-md ${config.borderGlow} group`}>
